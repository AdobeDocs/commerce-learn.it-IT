...
description: Scopri l’elenco di controllo pre-avvio di Adobe Commerce Cloud.
feature: Cloud
...
# Elenco di controllo pre-lancio Commerce Cloud

Di seguito è riportata una sintesi della [documentazione di lancio del sito](https://experienceleague.adobe.com/it/docs/commerce-cloud-service/user-guide/launch/overview){target="_blank"} di Adobe Commerce.
...
- Alcuni controlli sono evidenziati con [!BADGE Blocker]{type=caution tooltip="Potenziale bloccante"} in quanto potrebbero bloccare il lancio se non vengono esaminati attentamente.
...
1. Consulta la documentazione sui test e la pubblicazione della [documentazione di lancio del sito](https://experienceleague.adobe.com/it/docs/commerce-cloud-service/user-guide/launch/overview){target="_blank"}
...
2. [!BADGE Blocco]{type=caution tooltip="Potenziale bloccante"} Rivedi i consigli e le informazioni di Support Insights (SWAT) ([Guida utente](https://experienceleague.adobe.com/it/docs/commerce-operations/tools/site-wide-analysis-tool/intro){target="_blank"})
...
4. Il team di integratori di sistemi ha eseguito UAT end-to-end su staging e produzione. Consulta la [documentazione di Experience League](https://experienceleague.adobe.com/it/docs/commerce-cloud-service/user-guide/develop/test/staging-and-production){target="_blank"}.
5. Confermare la distribuzione e il test del codice negli ambienti di staging e produzione ([Ulteriori informazioni](https://experienceleague.adobe.com/it/docs/commerce-cloud-service/user-guide/develop/test/staging-and-production){target="_blank"}).
...
1. Aggiorna Adobe Commerce e i pacchetti/servizi correlati alla [versione più recente](https://experienceleague.adobe.com/it/docs/commerce-operations/release/notes/overview){target="_blank"}
2. Rivedi le configurazioni e i servizi correnti con il tuo SI/Partner e [segui le best practice](https://experienceleague.adobe.com/it/docs/commerce-operations/implementation-playbook/best-practices/planning/catalog-management){target="_blank"}.
3. Rivedi MySQL/Shared-Files [utilizzo disco](https://experienceleague.adobe.com/it/docs/commerce-cloud-service/user-guide/develop/storage/manage-disk-space){target="_blank"}

## &#x200B;3. Configurazioni Fastly

1. [!BADGE Blocco]{type=caution tooltip="Potenziale bloccante"} Verificare che il caching funzioni ([Cache a pagina intera](https://developer.adobe.com/commerce/frontend-core/guide/caching/){target="_blank"} o [Memorizzazione cache di GraphQL](https://developer.adobe.com/commerce/webapi/graphql/usage/caching/){target="_blank"}). Leggi la [Guida alla configurazione rapida](https://experienceleague.adobe.com/it/docs/commerce-cloud-service/user-guide/cdn/fastly){target="_blank"}.
2. Utilizza il metodo GET per le query GraphQL sui siti web PWA/Headless, se applicabile.

   >[!NOTE]
   > È possibile memorizzare in cache solo le query inviate con un’operazione HTTP GET (se applicabile). Impossibile memorizzare nella cache [le query POST](https://developer.adobe.com/commerce/webapi/graphql/usage/caching/){target="_blank"}.

3. Assicurati che Ottimizzazione immagine rapida sia abilitato ([Vedi Ottimizzazione immagine rapida](https://experienceleague.adobe.com/it/docs/commerce-cloud-service/user-guide/cdn/fastly-image-optimization){target="_blank"})
4. Verificare che la posizione dello scudo corretta sia configurata ([Configura cache, backend e schermatura origine](https://experienceleague.adobe.com/it/docs/commerce-cloud-service/user-guide/cdn/setup-fastly/fastly-custom-cache-configuration){target="_blank"}).
5. Firewall applicazione Web (**WAF**) funziona. (Vedi [Risoluzione dei problemi relativi alle richieste bloccate](https://experienceleague.adobe.com/it/docs/commerce-cloud-service/user-guide/cdn/fastly-waf-service){target="_blank"}, se presenti, e limitazioni)
...
1. [!BADGE Blocco]{type=caution tooltip="Potenziale bloccante"} Verificare che tutti i nomi di dominio richiesti siano stati richiesti. _(Inviare in anticipo un ticket di supporto per tutti i domini aggiunti o modificati)_
2. [!BADGE Il certificato SSL (TLS) di blocco]{type=caution tooltip="Potenziale bloccante"} è stato applicato ai domini. Leggi [questo articolo](https://experienceleague.adobe.com/it/docs/commerce-knowledge-base/kb/how-to/ssl-tls-certificates-for-magento-commerce-cloud-faq){target="_blank"} per ulteriori informazioni.
3. Aggiorna il valore [TTL (Time to Live)](https://experienceleague.adobe.com/it/docs/commerce-cloud-service/user-guide/launch/checklist#to-update-dns-configuration-for-site-launch){target="_blank"} DNS al minimo possibile per il lancio.
4. Abilita Sendgrid SPF e DKIM

   >[!NOTE]
   > Aggiungere i record CNAME SendGrid per ogni dominio alla configurazione DNS. Leggi [SendGrid email service](https://experienceleague.adobe.com/it/docs/commerce-cloud-service/user-guide/project/sendgrid){target="_blank"} per scoprire come modificare i domini del mittente e altro ancora.
...
- Avvisi gestiti su Adobe Commerce: [Avvisi MariaDB](https://experienceleague.adobe.com/it/docs/commerce-knowledge-base/kb/support-tools/managed-alerts/managed-alerts-on-magento-commerce-mariadb-alerts){target="_blank"}
...
- Analisi approfondita per [repliche cluster Galera e controllo del flusso.](https://experienceleague.adobe.com/it/docs/commerce-learn/tutorials/backend-development/galera-db-slow-replication){target="_blank"}
...
1. Rivedi lo stato ideale dell’implementazione di contenuti statici (SCD) per ridurre i tempi di manutenzione durante le distribuzioni nell’ambiente di produzione. Rivedi [Strategie di distribuzione del contenuto statico](https://experienceleague.adobe.com/it/docs/commerce-cloud-service/user-guide/develop/deploy/static-content){target="_blank"} e [Guida alla gestione della configurazione dell&#39;archivio](https://experienceleague.adobe.com/it/docs/commerce-cloud-service/user-guide/configure-store/store-settings){target="_blank"}.
2. Rivedi le impostazioni di minimizzazione per HTML, JavaScript e CSS. (Questo non si applica ai siti web PWA/Headless).
3. Conferma che l’utilizzo delle seguenti variabili cloud sia in linea con le finalità previste. ([SCD_MATRIX](https://experienceleague.adobe.com/it/docs/commerce-cloud-service/user-guide/configure/env/stage/variables-build#scd_matrix){target="_blank"}, [SCD_ON_DEMAND](https://experienceleague.adobe.com/it/docs/commerce-cloud-service/user-guide/configure/env/stage/variables-global#scd_on_demand){target="_blank"} e [SKIP_SCD](https://experienceleague.adobe.com/it/docs/commerce-cloud-service/user-guide/configure/env/stage/variables-deploy#skip_scd){target="_blank"})

## &#x200B;7. Test e risoluzione dei problemi

1. Verifica le e-mail transazionali in uscita. Ulteriori informazioni sulla funzionalità [Adobe Commerce Cloud - SendGrid Mail](https://experienceleague.adobe.com/it/docs/commerce-cloud-service/user-guide/project/sendgrid){target="_blank"}.
2. [!BADGE Blocco]{type=caution tooltip="Potenziale bloccante"} Blocchi con Adobe?
3. [!BADGE Blocco]{type=caution tooltip="Potenziale bloccante"} Esegui test di carico e stress sull&#39;istanza di produzione prima di andare in diretta e condividi i risultati con il CTA/CSE assegnato.

   >[!NOTE]
   > Un [test di carico e stress serve allo scopo](https://experienceleague.adobe.com/it/docs/commerce-cloud-service/user-guide/develop/test/guidance#:~:text=A%20load%20test%20can%20help,Scan%20Tool%20for%20your%20sites.){target="_blank"} di identificare i colli di bottiglia e individuare i problemi di prestazioni all&#39;interno dell&#39;applicazione. Svolge un ruolo cruciale nella gestione delle aspettative relative alle dimensioni dei cluster e nella determinazione degli adeguamenti di scalabilità necessari per soddisfare i requisiti aziendali in modo efficace.

   >[!IMPORTANT]
   > **_WARNING:_** Durante la preparazione di un test di carico,_ **_non_** inviare e-mail di transazione live (anche a indirizzi fittizi). L’invio di e-mail durante il test può far raggiungere al progetto il limite di invio predefinito (12k) configurato per SendGrid prima dell’avvio.
   > 
...
3. Verificare che le configurazioni [SEO (Search Engine Optimization) siano configurate correttamente](https://experienceleague.adobe.com/it/docs/commerce-admin/marketing/seo/seo-overview){target="_blank"} per consentire agli indicizzatori/crawler di eseguire la scansione del sito Web, se necessario.
4. Aggiungere reindirizzamenti e route (vedere [Configurare le route](https://experienceleague.adobe.com/it/docs/commerce-cloud-service/user-guide/configure/routes/routes-yaml){target="_blank"})
...
5. Rimuovere gli utenti non più inclusi nel progetto (vedere [Creare e gestire gli utenti](https://experienceleague.adobe.com/it/docs/commerce-cloud-service/user-guide/project/user-access){target="_blank"}).
6. Le password per gli amministratori sono configurate (vedi [Requisiti per le password amministratore](https://experienceleague.adobe.com/it/docs/commerce-admin/systems/security/security-admin){target="_blank"}).
...
Quando è il momento di passare al cutover, esegui i seguenti passaggi (per ulteriori informazioni, consulta [Configurazioni DNS](https://experienceleague.adobe.com/it/docs/commerce-cloud-service/user-guide/launch/checklist){target="_blank"}):
...
In caso di problemi che impediscono l&#39;avvio durante il passaggio, il metodo più veloce per ottenere un supporto tempestivo adeguato consiste nell&#39;utilizzare l&#39;help desk e aprire un ticket con il motivo &quot;Impossibile avviare il mio negozio&quot; e chiamare un numero di assistenza della hotline (vedere [l&#39;elenco dei numeri della hotline Adobe Commerce P1 (Priorità 1)](https://support.magento.com/hc/en-us/articles/360042536151){target="_blank"}):
...
> - [Elenco di controllo avvio - Guida utente](https://experienceleague.adobe.com/it/docs/commerce-cloud-service/user-guide/launch/checklist){target="_blank"}
