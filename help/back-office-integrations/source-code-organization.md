...
* Le quattro cartelle principali: `actions`, `scripts`, `test` e `utils`, con lo stato attivo sulle cartelle `actions` e `scripts` durante la sessione. &#x200B;
...
* Scopri l’importanza di non modificare il codice nella cartella `starter-kit-info`, che contiene un’azione di runtime utilizzata da Adobe per monitorare le distribuzioni dei progetti in base al kit di avvio. &#x200B;
